		...
		<AppLayout breadcrumbs={breadcrumbs}>
			...
			<div className="flex h-full flex-1 flex-col gap-8 p-8">
				...
				<div className="grid gap-6 md:grid-cols-4">
					...
					<StatCard title="Completed orders" count={stats.completed} />
					<StatCard title="Cancelled orders" count={stats.cancelled} />
				</div>
					...
					<CardContent className="p-0">
						<div className="mb-6 flex flex-col items-center justify-between gap-4 md:flex-row">
							...
							<div className="flex gap-1 rounded-lg p-1">
								{["all", "in-progress", "complete", "cancelled"].map((tab) => (
									<button
										key={tab}
							...
							<Table>
								<TableHeader>
									<TableRow className="hover:bg-transparent">
										...
										<TableHead className="font-semibold">The product</TableHead>
								...
								<TableBody>
									{orders?.data?.map((order) => {
										...
										const productImage =
											firstItem?.product?.media?.[0]?.original_url || "";
										const productName =
											firstItem?.product?.name || "Unknown Product";

										return (
											<TableRow key={order.id} className="border-b/50 py-4">
							...
							<div className="mt-6 flex items-center justify-between">
								...
								<div className="flex gap-2">
									<Button
										...
										onClick={() =>
											orders.links[0].url && router.get(orders.links[0].url)
										}
									>
										Previous
									</Button>
									<Button
										...
										onClick={() =>
											...
											router.get(
												orders.links[orders.links.length - 1].url as string,
											)
										}
									>
										Next
